...
export default function HorizontalAbout({ team }: Props) {
  ...
  return (
    <div ref={containerRef} className="horizontal-about-wrapper" style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
        ...
        <div className="about-slide slide-values">
          <div className="slide-content">
            ...
            <div className="values-layout">
              {values.map((value) => (
                <div key={value.name} className="value-card glass-card">
              ...
              <div className="team-layout">
                {team.map((member) => (
                  <div key={member.id} className="team-card glass-card">
                    {member.data.photo ? (
                      <img src={member.data.photo} alt={member.data.name} loading="lazy" decoding="async" />
                    ) : (
                      <div className="team-avatar-placeholder">{member.data.name.charAt(0)}</div>
                    )}
