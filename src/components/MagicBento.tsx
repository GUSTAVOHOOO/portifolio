...
const DEFAULT_CARD_DATA: CardData[] = [
  { color: '#120F17', title: 'Sites Profissionais', description: 'Presença digital que converte visitantes em clientes', label: 'Web' },
  { color: '#120F17', title: 'Lojas Online', description: 'E-commerce completo para vender 24h por dia', label: 'E-commerce' },
  { color: '#120F17', title: 'Cardápios Digitais', description: 'Menu interativo para restaurantes e delivery', label: 'Food' },
  { color: '#120F17', title: 'Landing Pages', description: 'Páginas de alta conversão para campanhas', label: 'Marketing' },
  { color: '#120F17', title: 'SEO & Performance', description: 'Apareça no Google e carregue em segundos', label: 'SEO' },
  { color: '#120F17', title: 'Suporte Contínuo', description: 'Atendimento dedicado pós-entrega', label: 'Suporte' },
];
...
const MagicBento = ({
  ...
  const scopeClass = `mb-${scopeId}`;
  const scopedStyle = cardData
    .map(
      (card, i) =>
        `.${scopeClass} .magic-bento-card:nth-child(${i + 2}) { background-color: ${card.color}; --glow-color: ${glowColor}; }`
    )
    .join('\n');
  ...
  return (
    <>
      ...
      <div className={`card-grid bento-section ${scopeClass}`} ref={gridRef}>
